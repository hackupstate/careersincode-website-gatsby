...
const createServiceLock = async (programPath, serviceName, content) => {
  ...
  try {
    ...
    const unlock = await _properLockfile.default.lock(serviceDataFile, {
...
const getService = async (programPath, serviceName) => {
  if ((0, _ci.isCI)()) return memoryServices[serviceName] || null;
  const siteDir = getSiteDir(programPath);

  try {
    return JSON.parse(await _fsExtra.default.readFile(getDataFilePath(siteDir, serviceName), `utf8`));
  } catch (err) {
    return null;
  }
};
